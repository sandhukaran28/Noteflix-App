...
// ---------- pool & sdk clients (lazy so build-time validation doesn't break SSG) ----------
let _pool: CognitoUserPool | null = null;
const pool = (): CognitoUserPool => {
  if (!_pool) _pool = new CognitoUserPool({ UserPoolId: userPoolId, ClientId: clientId });
  return _pool;
};
let _cip: CognitoIdentityProviderClient | null = null;
const cip = (): CognitoIdentityProviderClient => {
  if (!_cip) _cip = new CognitoIdentityProviderClient({ region });
  return _cip;
};
